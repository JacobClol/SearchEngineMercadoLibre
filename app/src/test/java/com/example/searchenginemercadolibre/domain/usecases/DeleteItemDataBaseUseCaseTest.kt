...
class DeleteItemDataBaseUseCaseTest {
    @RelaxedMockK
    ...
    @Test
    fun `when deleteItemDataBaseUseCase is invoke then call deleteItemsFromDataBase with the item and invoke deleteAttributeDataBaseUseCase`() =
        runBlocking {
            //Given
            val itemMock = Item(
                itemId = "MCO811601010",
            ...
            //Then
            coVerify(exactly = 1) {
                deleteAttributeDataBaseUseCase(itemMock.itemId)
                deleteItemDataBaseUseCase(itemMock)
            }
        }
}